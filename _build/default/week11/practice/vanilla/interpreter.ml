...
let rec interp_e (s : Store.t) (e : Ast.expr) : Store.value = 
  ...
  | Ast.Cond (cond, t_expr, f_expr) -> let cond_interp = interp_e s cond in
                                       let t_expr_interp = interp_e s t_expr in
                                       let f_expr_interp = interp_e s f_expr in
                                       begin
                                        match cond_interp with
                                        | Store.BoolV condition -> if condition = true then
                                                                      t_expr_interp
                                                                   else
                                                                      f_expr_interp
  ...
  | Ast.Lt (lhs, rhs) -> let lhs_interp = interp_e s lhs in
                         ...
                         begin
                          ...
                          | (Store.NumV l_nv, Store.NumV r_nv) -> if l_nv < r_nv then
                                                                      interp_e s (Ast.Bool true)
                                                                  else 
                                                                      interp_e s (Ast.Bool false)
                                                                      
                          | _ -> failwith (F.asprintf "Invalid Compare : %a < %a" Ast.pp_e lhs Ast.pp_e rhs)
                         end
